...
export default function ProductsClient({ categories }: { categories: SanityCategory[] }) {
  const [openCategory, setOpenCategory] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  const allSearchableProducts = useMemo(() =>
    categories
      .filter((cat) => cat.categoryId !== "engineering-services")
      .flatMap((cat) =>
        cat.subcategories.flatMap((sub) =>
          sub.products
            .filter((p) => !p.comingSoon)
            .map((p) => ({ ...p, categoryTitle: cat.title, categoryId: cat.categoryId }))
        )
      ), [categories]);
  ...
  const searchResults = useMemo(() => {
    ...
    if (!q) return [];
    return allSearchableProducts.filter((p) => p.name.toLowerCase().includes(q));
          ...
          <section className="products-categories">
            <div className="products-container">
              {categories.map((category, idx) => {
                const isOpen = openCategory === category.categoryId;
                return (
                  <motion.div
                  ...
                  >
                    <div
                      ...
                      onClick={() => setOpenCategory(isOpen ? null : category.categoryId)}
                    >
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          ...
                          {category.categoryId === "iest-products" && (
                          ...
                          {category.subcategories.map((sub) => {
                            ...
                            return (
                              <div key={sub.name} className="subcategory-block">
                                ...
                                {category.categoryId === "engineering-services" ? (
                          ...
                          <div className="category-cta">
                            ...
                            <Link href="/contact" className="category-cta-link">Contact our team →</Link>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
